...
const profilePreview = document.getElementById('profile-preview'); // NOVO
const profilePreviewContainer = document.getElementById('profile-preview-container'); // NOVO

// Botões de Navegação
const logoutButton = document.getElementById('logout-button');
const backButton = document.getElementById('back-button'); // NOVO

// --- VARIÁVEL PARA O LINK DA FOTO (Não é mais necessária como global, mas mantida para referência) ---
...
// NOVO: Função para Voltar
function goBack() {
    if (screenHistory.length > 1) {
        // Remove a tela atual
        screenHistory.pop();
        // Obtém a tela anterior
        const prevScreenId = screenHistory[screenHistory.length - 1];
        // Mostra a tela anterior, sinalizando que é uma ação de voltar
        showScreen(prevScreenId, true);
    }
}

// NOVO: Função para pré-visualizar a imagem
...
function applyTranslations() {
    ...
    // Main App
    // ... (O restante das traduções permanece o mesmo)
...
function updateCart() {
    // ... (Função updateCart permanece a mesma)
...
// Monta a mensagem do WhatsApp e redireciona
function sendOrderToWhatsApp() {
    if (cart.length === 0) return;
    // ... (Função sendOrderToWhatsApp permanece a mesma)
...
// Função para Sair da Conta e limpar registros
function logoutUser() {
...
// Listener para pré-visualização de foto
profilePictureInput.addEventListener('change', () => {
    applyTranslations(); // Atualiza o texto e chama previewProfilePicture
});


// Listener de Login/Registro com Firebase Storage e Firestore
registrationForm.addEventListener('submit', async (e) => {
    ...
    if (profilePictureFile) {
        try {
            ...
            alert("Erro ao carregar a foto de perfil. Tente novamente.");
            return; 
        }
    }
    ...
    // Força a exibição da tela inicial, que será o primeiro item do histórico
...
// NOVO: Listener para o botão de Voltar
if (backButton) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (isRegistered === 'true') {
        ...
        loginSection.classList.add('hidden');
        mainAppSection.classList.remove('hidden');
        // A tela inicial (home-screen) deve ser o primeiro item do histórico
        showScreen(homeScreenId);
    } else {
        // Se não registrado, mostra a tela de login e garante que o histórico esteja vazio
        loginSection.classList.remove('hidden');
        mainAppSection.classList.add('hidden');
        screenHistory = [];
    }
